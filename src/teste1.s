...
B: resd 1
C: resd 1
QTD_TERMOS_FIBONACCI: resd 1

section .text
global _start
_start:
push QTD_TERMOS_FIBONACCI
call read_int
push ebx
mov ebx, [ONE]
mov dword [A], ebx
...
mov eax, [QTD_TERMOS_FIBONACCI]
sub dword eax, [ONE]
mov dword [QTD_TERMOS_FIBONACCI], eax
cmp EAX,0 
jg FIBONACCI
PARAR: mov eax, 1
mov ebx, 0
...
read_digit:                       
    ...
    je      end_reading             ; Se for enter, fim  
    pop     EAX             
    inc     EAX
    push    EAX                     ; conta char ++
    ...
    jmp     end_reading         ; Se chegou no limite, fim
read_negative:
    pop     EAX             
    inc     EAX
    push    EAX                 ; char ++
